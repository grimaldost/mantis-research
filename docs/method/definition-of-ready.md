...
context, not a machine verdict). [keel](https://github.com/grimaldost/keel)'s `check-ready` gate enforces both halves: it passes
...
### Reference: what keel's `check-ready` well-formedness checks assert
...
- [ ] A pre-mortem pass has been run by a non-author reviewer, and the certification
      ...
      (operator-accepted, ready modulo a named fix; keel's `check-ready` gate passes with a WARN, not EXIT 1).
      *(keel's `check-ready` gate enforces this — B1.)*
...
pre-mortem certification is recorded. keel's `check-ready` gate enforces both halves; the
